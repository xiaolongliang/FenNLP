...
# Data
loader = DataLoader("imdb", batch_size, epoch, is_training=True,
                    use_mask=True,use_segment=True,
                    mode="classfication")
...
class Model(tf.keras.Model):
    ...
    def call(self, inputs, masks=None,
        ...
        model = BERTModel(config=bert_config_file,
...
model = Model(bert_config_file, use_one_hot_embeddings=use_one_hot_embeddings)
...
Trainer.train(
    ...
    loss=loss,  # 损失函数
    is_training=is_training,  # 是否是训练
    do_valid=do_valid,
    init_checkpoint=init_checkpoint)  # 是否使用预训练权重
